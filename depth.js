...
var median_descs_long = [
    ...
    "One property of the 1D median is that it is in the maximum number of open intervals made from pairs of points in the point set. An interval in 1D is a special case of a <b>simplex</b>, and in 2D a simplex is a triangle. The simplicial median of a point set is a point in the plane contained in the most triangles created by triples of points from the point set. Here, for performance reasons, only points in the point set are considered.",
    "A point in 1D cuts the real line into 2 halves. One side of such a split is called a <b>halfspace</b>. The 1D median of a point set S has the property that there are an equal number of points from S in the halfspaces on either side of it. <br><br>In 2D, there are an infinite number of ways to split the plane into halfspaces with a line through a point. The halfspace depth of a point p in a set S is the minimum number of points from S on one side of <i>any</i> halfspace through p. Note that the 1D, the median has maximal halfspace depth. So, in 2D, we define the halfspace median of S to be the point in the plane with maximum halfspace depth. Here, for performance reasons, only points in the point set are considered."
];
